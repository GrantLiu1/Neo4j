...
# from py2neo import *
#
# #driver = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", "187811"))  # 认证连接数据库
#
# driver=Graph("http://localhost:7474",username="neo4j",password="187811")
#
...
# @app.route('/graph')  # 两个路由指向同一个网页，返回图的节点和边的结构体
# def get_graph():
#     # Nmatch=NodeMatcher(driver)
#     # node=match()
#     nodeList = []
#     edgeList = []
#     results=driver.run('MATCH (p1{name:"Laurence Fishburne"})-[r1:ACTED_IN]->(m)<-[r2:DIRECTED]-(p2)  RETURN p1,m,p2,r1,r2').data()
#     for result in results:
#         nodeList.append(result[0])
#         nodeList.append(result[1])
#         nodeList.append(result[2])
